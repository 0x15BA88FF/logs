...
export async function getAllBlogs(): Promise<BlogMetadata[]> {
  const slugs = fs
    .readdirSync(path.resolve("src/lib/content/blog/posts"))
    .filter((file) => file.endsWith(".md"))
    .map((file) => file.replace(/\.md$/, ""));

  const all = await Promise.all(slugs.map(importBlog));
  return all.filter((b): b is BlogMetadata => b !== null).sort(sortByDateDescending);
}
